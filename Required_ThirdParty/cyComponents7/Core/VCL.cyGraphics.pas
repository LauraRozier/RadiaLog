...
uses Classes, Windows, Forms, Graphics, Math, Buttons, Controls, ExtCtrls, Jpeg, {$IFDEF DELPHI2009_OR_ABOVE} pngimage, {$ENDIF}
      StdCtrls, SysUtils, VCL.cyTypes;

// Background functions :
procedure cyGradientFill(aCanvas: TCanvas; aRect: TRect; fromColor, toColor: TColor; adgradOrientation: TdgradOrientation; Balance, AngleDegree: Word; balanceMode: TDgradBalanceMode; Maxdegrade: Byte; SpeedPercent: Integer; const AngleClipRect: Boolean = true; const AngleBuffer: TBitmap = Nil);
...
procedure cyGradientFillAngle(aCanvas: TCanvas; aRect: TRect; fromColor, toColor: TColor; MaxDegrad: Byte; AngleDegree: Word; const ClipRect: Boolean = true; const Buffer: TBitmap = Nil);
...
procedure cyDrawSpeedButtonFace(Canvas: TCanvas; var Rect: TRect; GradientColor1, GradientColor2: TColor; aState: TButtonState; Focused, Hot: Boolean);
procedure cyDrawSpeedButton(Canvas: TCanvas; Caption: String; ARect: TRect; GradientColor1, GradientColor2: TColor; aState: TButtonState; Focused, Hot: Boolean);
...
function GetScreenPixelColor(ScreenCursorPos: TPoint; var RslColor: TColor): Boolean;
  ...
  begin
    case adgradOrientation of
      ...
      dgdAngle:
        begin
          ...
          cyGradientFillAngle(aCanvas, aRect, fromColor, toColor, MaxDegrade, AngleDegree, AngleClipRect, aBmp);

          if AngleBuffer = Nil then
            aBmp.Free;
        end;
    ...
    case adgradOrientation of
      ...
      begin
        ...
        IntermediateRect := classes.Rect(0, 0, MulDiv(Balance, Size, 100), Size);
        ...
        try
          if aBmp.Width  <> Size then
            aBmp.Width := Size;
          if aBmp.Height <> Size then
            aBmp.Height := Size;

          case BalanceMode of
            bmNormal:
            begin
              cyGradientFillHorizontal(aBmp.Canvas, IntermediateRect, fromColor, RGB(Arr_Med1RGB[0], Arr_Med1RGB[1], Arr_Med1RGB[2]), MaxDegradeBalance1);
              IntermediateRect.Left := IntermediateRect.Right;
              IntermediateRect.Right := Size;
              cyGradientFillHorizontal(aBmp.Canvas, IntermediateRect, RGB(Arr_Med2RGB[0], Arr_Med2RGB[1], Arr_Med2RGB[2]), toColor, MaxDegradeBalance2);
            end;

            bmMirror:
            begin
              cyGradientFillHorizontal(aBmp.Canvas, IntermediateRect, fromColor, toColor, MaxDegradeBalance1);
              IntermediateRect.Left := IntermediateRect.Right;
              IntermediateRect.Right := Size;
              cyGradientFillHorizontal(aBmp.Canvas, IntermediateRect, toColor, FromColor, MaxDegradeBalance2);
            end;
            ...
            begin
              ...
              IntermediateRect.Left := IntermediateRect.Right;
              IntermediateRect.Right := Size;
              cyGradientFillHorizontal(aBmp.Canvas, IntermediateRect, toColor, RGB(Arr_Med2RGB[0], Arr_Med2RGB[1], Arr_Med2RGB[2]), MaxDegradeBalance2);
            end;
            ...
            bmInvertReverse:
            begin
              cyGradientFillHorizontal(aBmp.Canvas, IntermediateRect, RGB(Arr_Med2RGB[0], Arr_Med2RGB[1], Arr_Med2RGB[2]), toColor, MaxDegradeBalance1);
              IntermediateRect.Left := IntermediateRect.Right;
              IntermediateRect.Right := Size;
              cyGradientFillHorizontal(aBmp.Canvas, IntermediateRect, RGB(Arr_Med1RGB[0], Arr_Med1RGB[1], Arr_Med1RGB[2]), fromColor, MaxDegradeBalance2);
            end;
...
begin
  ...
  try
    ...
    cyGradientFillHorizontal(aBmp.Canvas, classes.Rect(0, 0, Size, Size), fromColor, toColor, MaxDegrad);
...
begin
  ...
  if (fromColor <> clNone) or (toColor <> clNone) then
    cyGradientFill(Canvas, Rect, fromColor, toColor, dgdVertical, 50, 0, balanceMode, 255, 95);
...
begin
  ...
  // Draw SpeedButton body if not transarent:
  if (fromColor <> clNone) or (toColor <> clNone) then
    cyGradientFill(Canvas, Rect, fromColor, toColor, dgdVertical, 50, 0, balanceMode, 255, 95);
end;
...
var
  ...
  savStyle: TBrushStyle;
begin
  cyDrawSpeedButtonFace(Canvas, ARect, GradientColor1, GradientColor2, aState, Focused, Hot);
...
begin
  SetStretchBltMode(Destination.Handle, HALFTONE); // HALFTONE/STRETCH_HALFTONE for high quality!
...
begin
  ...
  then begin
    ...
    then
      DrawIconEx(Destination.Handle, DestRect.Left, DestRect.Top, TIcon(aGraphic).Handle, DestRect.Right-DestRect.Left, DestRect.Bottom-DestRect.Top, 0, 0, DI_Normal)
    else
...
begin
  ...
  then begin
    ...
    {$ELSE}
      Windows.DrawText(CanvasHandle, PChar(Text), -1, CalcRect, CalcFlags);
    {$ENDIF}
  ...
  {$ELSE}
    Windows.DrawText(CanvasHandle, PChar(Text), -1, Rect, TextFormat);
  {$ENDIF}
end;
...
begin
  ...
  Canvas.Pen.Color := ColorModify(PageColor, -40, -40, -40);
...
begin
  ...
  if DesktopDC <> 0 then
  begin
    RslColor := GetPixel(DesktopDC, ScreenCursorPos.x, ScreenCursorPos.y);
    Result := true;
  end;
