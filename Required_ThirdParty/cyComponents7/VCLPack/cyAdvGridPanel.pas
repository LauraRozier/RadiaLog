{   Component(s):
    ...
    Description:
    A GridPanel that paint a graphic (Bitmap, jpeg etc ...)
...
uses VCL.cyClasses, cyGridPanel, Windows, Themes, ExtCtrls, Graphics, classes, Messages, Controls;

type
  TcyAdvGridPanel = class(TcyCustomGridPanel)
  private
...
constructor TcyAdvGridPanel.Create(AOwner: TComponent);
begin
...
destructor TcyAdvGridPanel.Destroy;
begin
  FWallpaper.Free;
  inherited Destroy;
end;

procedure TcyAdvGridPanel.DrawBackground(aRect: TRect);
begin
  Inherited;    // Draw degrade ...
  cyDrawBgPicture(Canvas, aRect, FWallpaper);
end;

procedure TcyAdvGridPanel.SetWallpaper(const Value: TcyBgPicture);
begin
  FWallpaper := Value;
end;
