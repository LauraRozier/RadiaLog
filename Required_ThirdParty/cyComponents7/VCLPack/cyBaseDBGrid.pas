{
  Note: ClientColumn not working if DBGrid without column defined and open a clientDataset (with Createdataset for exemple) at run-time.
        Calling AdjustClientColumn after open the dataset dos not solve the problem.
}
...
uses Classes, Windows, Controls, Graphics, Grids, DBGrids, Messages, Db, jpeg, ExtCtrls, StdCtrls, ComCtrls, RichEdit, Forms, StrUtils,
      Themes, VCL.cyTypes, VCL.cyGraphics,
...
type
  ...
  TFieldContentRendering = (fcDefault, fcWordwrap, fcMemo, fcBitmap, fcJpeg, fcIcon, fcPng, fcCheckBox, fcCheckBoxWithText);
  ...
  TBooleanFieldDisplay = (bfDefault, bfCheckBox, bfCheckBoxWithText);
  TGraphicFieldDisplay = (gfDefault, gfImageFormat, gfImage);
  ...
  private
    ...
    FOnTitleDblClick: TDBGridClickEvent;
    FOnCellDblClick: TDBGridClickEvent;
    FCorrectVerticalScrollBar: Boolean;
    {$IFDEF SCROLL_FEATURE}
    ...
    procedure SetColumnIndex(const Value: Integer);
    // 2013/10/30 no more needed ... procedure WMNCCalcSize(var Msg: TMessage); message WM_NCCALCSIZE;
  ...
  protected
    ...
    // 2013/10/30 no more needed ... procedure WndProc(var Message: TMessage); override;
    ...
    {$ENDIF}
    property OnTitleDblClick: TDBGridClickEvent read FOnTitleDblClick write FOnTitleDblClick;
    ...
    property CorrectVerticalScrollBar: Boolean read FCorrectVerticalScrollBar write FCorrectVerticalScrollBar default true;
  public
    ...
    procedure DrawBooleanField(Rect: TRect; Field: TField; const DrawTextField: Boolean);
    //
...
const
  ...
  procedure AdjustColumnsToContent(aDBGrid: DBGrids.TDBGrid; const MaxRows: Integer = 1000; const incColumnWidth: Integer = 8);
...
var
  ...
  Bookmark: TBookmark;
      ...
      function SpecialRendering(aFieldname: string): boolean;
      begin
        Result := false;
      end;
...
begin
  ...
  begin
    ...
    // Initializations :
    MaxColumnWidth := aDBGrid.ClientWidth;
    RowCounter := 0;
    ...
    for C := 0 to Columns.Count-1 do
      ...
      begin
        Array_ColWidths[C] := (-1) * aDBGrid.Columns[c].Width;

        if aDBGrid.Columns[C].Fieldname <> '' then   // No field associated !
          if not SpecialRendering(aDBGrid.Columns[C].Fieldname) then
            if (not (aDBGrid.Columns[C].Field is TGraphicField)) and (not (aDBGrid.Columns[C].Field is TMemoField)) and (not (aDBGrid.Columns[C].Field is TBlobField)) then
            begin
    ...
    begin
      ...
      for C := 0 to aDBGrid.Columns.Count - 1 do
        if aDBGrid.Columns[C].Visible then
          if aDBGrid.Columns[C].Fieldname <> '' then
            if Array_ColWidths[C] >= 0 then       // Not Wordwrap text and no Graphic/Blob/memo field ...
            begin
              ...
              FieldValueWidth := Canvas.TextWidth(aDBGrid.Columns[C].Field.Text);  // .Text allow better result than  .AsString because of DisplayFormat
...
begin
  ...
  begin
    ...
    if not FGrid.ClientColumn.Enabled then
    begin
      FGrid.BeginLayout;
      FGrid.LayoutChanged;
      FGrid.EndLayout;
...
begin
  if FActiveDataRowHeight <> Value then
  begin
    FActiveDataRowHeight := Value;

    if not (csLoading in FGrid.ComponentState) then   // 2014-11-20 Avoid error copy/paste DBGrid when ActiveDataRowHeight <> 0
      FGrid.cyUpdateLayout;
  end;
end;

procedure TcyCustomLayoutOptions.SetTitleRowHeight(const Value: Integer);
begin
  if FTitleRowHeight <> Value then
  begin
    FTitleRowHeight := Value;
    //   2015-09-24 No changes at design time ...      FGrid.cyUpdateLayout;
    FGrid.BeginLayout;
    FGrid.EndLayout;
  end;
end;
...
//    ScrollBars := ssHorizontal;
  ...
  FCorrectVerticalScrollBar := true;
  FSavInplaceEdit  := Nil;
  FMouseOverCell.X := -1;
  FMouseOverCell.Y := -1;
  FMouseJob := mjWait;
...
begin
  if ARow <> Row then
    ...
    begin
      // cyUpdateLayout;  // Can' t use here because Scroll(0) will raise stack overflow error (it is not a problem because there' s always a row active and oher' s not, RowCount don' t change) !!!

// 2015-10-06 Seems not be needed ...
//      BeginLayout;
//      LayoutChanged;
//      EndLayout;
    end;
...
begin
  ...
  Invalidate;      // 2014-05/26 Refresh checkboxes view state
end;
...
begin
  ...
  begin
    ...
    // Set Boolean fields :
    if (ColumnIndex > -1) and (FContentFieldsRender.FBooleanField in [bfCheckBox, bfCheckBoxWithText]) then
...
procedure TcyBaseDBGrid.DblClick;
var
  Cell: TGridCoord;
...
begin
  ...
  if (FGridState = gsRowSizing) or (FGridState = gsColSizing) or
    ((InplaceEditor <> nil) and (InplaceEditor.Visible) and
     (PtInRect(InplaceEditor.BoundsRect, pt))) then Exit;
  ...
  if (Cell.X >= IndicatorOffset) and (Cell.Y >= 0) then
    ...
    begin
      if Assigned(FOnTitleDblClick) then
        FOnTitleDblClick(Columns[RawToDataColumn(Cell.X)]);
    end
    else begin
      if Assigned(FOnCellDblClick) then
        FOnCellDblClick(Columns[SelectedIndex]);
    end;
end;
...
begin
  if FDefaultDrawingCheckBox then
    DefaultDrawCheckBox(Rect, Checked);

  if Assigned(FOnDrawCheckBox) then
    FOnDrawCheckBox(Self, Rect, Checked);
end;
...
begin
  ...
  begin
    if Checked
    then Details := ThemeServices.GetElementDetails(tbCheckBoxCheckedNormal)
    else Details := ThemeServices.GetElementDetails(tbCheckBoxUnCheckedNormal);
...
begin
  if not FClientColumn.FEnabled then Exit;
...
begin
  ...
  _ClientWidth := ClientWidth;     // May not consider Vertical scrollBar width if not visible!
...
procedure TcyBaseDBGrid.cyUpdateLayout;
begin
  // 2014-11-07 Best trick because will call UpdateRowCount + UpdateScrollBar + UpdateActive !!!
  Scroll(0);

(* No more needed with Scroll(0); trick !!!
...
procedure TcyBaseDBGrid.DrawBooleanField(Rect: TRect; Field: TField; const DrawTextField: Boolean);
var
...
//      if not (dgColumnResize in Options)   // TDBGrid don' t let moving columns ...
  ...
  // Set Boolean fields :
  if (not Handled) and (ColumnIndex > -1) and (FContentFieldsRender.BooleanField in [bfCheckBox, bfCheckBoxWithText]) then
    if Columns[ColumnIndex].Field is TBooleanField then
    begin
      EditorMode := false;
...
begin
  ...
  if FMouseJob = mjPrepareMovingRows then
    if MouseCoord(X, Y).Y <> FMouseDownCell.Y then    // Another row?
    begin
      AcceptMove := true;
...
procedure TcyBaseDBGrid.WMSize(var Message: TWMSize);
    ...
    procedure HideScrollBarsView;
    var
      VertScrollBarVisible, HorzScrollBarVisible: Boolean;
      wl: Integer;
    begin
      // if not CorrectVerticalScrollBar then Exit;
      ...
      begin
        ...
        begin
          ...
          if HorzScrollBarVisible <> FHorzScrollBar.Visible then
        ...
        begin
          ...
          if VertScrollBarVisible <> FVertScrollBar.Visible then
            ShowScrollBar(Handle, SB_VERT, FVertScrollBar.Visible);   // RHR may raise error if visible ...
        end;
      end;
    end;
...
begin
  ...
  begin
    ...
    if FCustomLayoutOptions.FActiveDataRowHeight <> 0 then
      for R := 1 to RowCount - 1 do
        if R = Row then
          RowHeights[R] := FCustomLayoutOptions.FActiveDataRowHeight;
  end;
...
{$IFDEF SCROLL_FEATURE}
// Called on TCustomDBGrid.CreateWnd, TCustomDBGrid.DataChanged, TCustomDBGrid.LinkActive and TCustomDBGrid.Scroll()
...
var
  ...
  rc, wl: Integer;
begin
  if not CorrectVerticalScrollBar then
    Inherited    // Only handle vertical scrollbar !
  else
    if (DataLink.Active) and (HandleAllocated) then
      with Datalink.DataSet do
      begin
        // *** Embarcadero's UpdateScrollBar original code :
        ...
        begin
          ...
          ScrollBarVisible := FVertScrollBar.Visible;
          // original :       ScrollBarVisible := RecordCount > 1;

          if ScrollBarVisible then
          begin
            // <Modified>
            if rc <= 1 then    // Avoid Index out of range when calling ShowScrollBar() twice ...
            begin
              SINew.nMin := 0;
              SINew.nPage := 0;
              SINew.nMax := 4;
        ...
        // End of Embarcadero's UpdateScrollBar original code *** //
      end;


  // *** FTimerScroll handling *** //
  // If any field data changed, UpdateScrollBar is also called.
  // So, we need to control if we have really moved to another record.
  if (not DataLink.Active) or (not HandleAllocated) then Exit;

  if DataLink.DataSet.RecNo <> LastScrollRecNo then
  begin
...
begin
  { 2014-12-04
     ...
     It seems affect details datasets when for one Master record there's no details records.
     Moving to another Master record with details, vertical scrollbar didn't restored on detail dataset attached DBGrid.

     2015-09-23 if SetRange returns single record and calling CancelRange, it created an error (see error below) !
  }
  ...
  begin
    ...
    ShowScrollBar(Self.Handle, SB_VERT, ScrollBarVisible);     // 2015-09-23 BUG when viewing one 1 record to several after calling CancelRange ...

    if ScrollBarVisible then
      SetScrollInfo(Self.Handle, SB_VERT, SINew, True);
   // End of Embarcadero's UpdateScrollBar original code *** //

    // It seems that if we call twice the code (with Inherited and with copied original code here), the problem disappear !!!
    if ScrollBarVisible then
      begin
        ShowScrollBar(Self.Handle, SB_VERT, ScrollBarVisible);
        SetScrollInfo(Self.Handle, SB_VERT, SINew, True);
