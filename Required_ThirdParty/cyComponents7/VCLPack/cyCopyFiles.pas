...
{$IFDEF UNICODE}
function FileAge(aFile: String): TDateTime;
begin
  SysUtils.FileAge(aFile, Result);
end;
{$ENDIF}
...
begin
  ...
  then
    try
      ...
      {$ELSE}
      if Windows.CopyFileEx(PAnsiChar(FromFile), PAnsiChar(ToFile), @CopyProgress, aProgressBar, @FCancCopyFileEx, 0)
      {$ENDIF}
      then begin
        if ResetAttr
        then FileSetAttr(ToFile, 0);
