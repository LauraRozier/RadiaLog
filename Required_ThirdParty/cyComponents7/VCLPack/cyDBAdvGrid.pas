...
uses Classes, Forms, Windows, Controls, Graphics, Math, Db, Grids, DBGrids, ImgList, stdCtrls, sysUtils, VCL.cyTypes, VCL.cyClasses, VCL.cyGraphics, cyBaseDBGrid, dialogs;
      ...
      try
        ...
        begin
          ...
          if ContentFieldsRender.BooleanField = bfCheckBoxWithText then
            if Column.Field is TBooleanField then
              FieldContentRendering := fcCheckBoxWithText;
          ...
          if (FieldContentRendering = fcDefault) and (not (csDesigning in ComponentState)) then  // Avoid draw Blob in design time becaus ewe don' t know image class
          ...
          if Assigned(FOnSetContentFieldRendering) then
            FOnSetContentFieldRendering(Self, ACol, ARow, ARect, Column, AState, Highlight, FieldContentRendering);
...
var
  ...
  Bookmark: TBookmark;
      ...
      function GetFieldContentRendering(aField: TField): TFieldContentRendering;
      begin
        Result := fcDefault;
  ...
  begin
    ...
    for C := 0 to Columns.Count-1 do
      ...
      begin
        ...
        begin
          Array_ColFieldContentRendering[C] := GetFieldContentRendering(Columns[C].Field);
