...
type
  ...
  public
    ...
    procedure Execute(Destination: TStrings; const fromCursor: Boolean = false); overload;
...
procedure TcyDatasetCsvExport.Execute(Destination: TStrings; const fromCursor: Boolean);
var
  f: Integer;
  ExportField, ExportRecord: Boolean;
  ...
  try
    ...
    begin
      ...
      if array_exportfield[f] then
        case FTTitlesMode of
          tmDisplayLabel: CsvLineAdd(FDataset.Fields[f].DisplayLabel, f = 0);
          tmFieldName :   CsvLineAdd(FDataset.Fields[f].FieldName, f = 0);
        end;
    end;
    ...
    begin
      ...
      begin
        ...
        for f := 0 to FDataset.FieldCount-1 do
        begin
          if FDataset.Fields[f].DataType in [ftMemo, ftFmtMemo {$IFDEF DELPHI2009_OR_ABOVE}, ftWideMemo{$ENDIF}] then
