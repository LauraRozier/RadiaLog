...
type
  ...
  protected
    InvalidSavedFieldValue: Boolean;
    SavedFieldValue: String;
...
begin
  ...
  InvalidSavedFieldValue := true;
  SavedFieldValue := '';

  (*   FDataLink.OnDataChange fired after :
         - Open the dataset
       ...
       FDataLink.OnEditingChange fired after :
         - Edit record
         - Save record
         - Cancel record   *)
...
begin
  if (FDataLink.Field <> nil) and (FEnabled) and (not (csDesigning in ComponentState)) then
  begin
    // 2014-12-03 Check if the field value is really different :
    if (SavedFieldValue = FDataLink.Field.AsString) and (not InvalidSavedFieldValue) then
      Exit;

    SavedFieldValue := FDataLink.Field.AsString;
    InvalidSavedFieldValue := false;

    if Assigned(FCustomChangeValue) then  // Allow developer to control assignment value to the control ...
      FCustomChangeValue(Self);

    if (FComponent <> nil) and (FComponentProperty <> '') then
    begin
