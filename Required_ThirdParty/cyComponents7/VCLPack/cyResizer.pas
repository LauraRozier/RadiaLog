...
uses Classes, Forms, Windows, SysUtils, RTLConsts, Controls, Graphics, Messages, VCL.cyTypes, VCL.cyGraphics, Dialogs;
...
type
  ...
  TProcGetHandlingMouseInformation = procedure (Sender: TObject; aPoint: TPoint; var aControl: TControl; var ControlRect, HandlerRect: TRect; var Job: TMouseJob) of object;
  TProcGetHandlingKeyInformation = procedure (Sender: TObject; Key: Word; ShiftState: TShiftState; KeyDown: Boolean; var Job: TKeyJob) of object;
  ...
  private
    ...
    FOnGetHandlingKeyInformation: TProcGetHandlingKeyInformation;
    FOnGetHandlingMouseInformation: TProcGetHandlingMouseInformation;
  ...
  public
    ...
    procedure DoGetHandlingMouseInformation(aPoint: TPoint; var aControl: TControl; var ControlRect, HandlerRect: TRect; var Job: TMouseJob);
    procedure DoGetHandlingKeyInformation(Key: Word; ShiftState: TShiftState; KeyDown: Boolean; var Job: TKeyJob);
  ...
  published
    ...
    property OnGetHandlingMouseInformation: TProcGetHandlingMouseInformation read FOnGetHandlingMouseInformation write FOnGetHandlingMouseInformation;
    property OnGetHandlingKeyInformation: TProcGetHandlingKeyInformation read FOnGetHandlingKeyInformation write FOnGetHandlingKeyInformation;
...
procedure TcyResizer.DoGetHandlingMouseInformation(aPoint: TPoint; var aControl: TControl;
...
begin
  ...
  then begin
    ...
    DoGetHandlingMouseInformation(FMouseHandlingFromPoint, aHandlingControl, aHandlingControlRect, HandlerRect, aMouseJob);
    ...
    begin
      ...
      FLastMouseDownControl := aHandlingControl;
    // *** Attention end *** //

    // 2014-07-4 Let user change information :
    if Assigned(FOnGetHandlingMouseInformation) then
      FOnGetHandlingMouseInformation(Self, FMouseHandlingFromPoint, FLastMouseDownControl, aHandlingControlRect, HandlerRect, aMouseJob);
...
begin
  ...
  then begin
    ...
    then begin
      ...
      DoGetHandlingMouseInformation(MousePosition, aControl, ControlRect, HandlerRect, Job);

      // 2014-07-04 Let user change information :
      if Assigned(FOnGetHandlingMouseInformation) then
        FOnGetHandlingMouseInformation(Self, MousePosition, aControl, ControlRect, HandlerRect, Job);
    end;
...
procedure TcyResizer.DoGetHandlingKeyInformation(Key: Word; ShiftState: TShiftState; KeyDown: Boolean; var Job: TKeyJob);
var Allow: Boolean;
begin
  ...
  if Assigned(FOnGetHandlingKeyInformation) then
    FOnGetHandlingKeyInformation(Self, Key, ShiftState, KeyDown, Job);
end;
...
begin
  ...
  then begin
    ...
    DoGetHandlingKeyInformation(Key, Shift, True, aJob);
    KeyHandlingJob := aJob;
...
begin
  Inherited;
  DoGetHandlingKeyInformation(Key, Shift, false, aJob);
  KeyHandlingJob := aJob;
...
procedure TcyResizer.CMDropFiles(var msg: TWMDropFiles);
begin
